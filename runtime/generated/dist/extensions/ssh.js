...
function stripRemotePathFromSshTarget(value) {
...
function normalizeKnownHostsKeychain(value) {
    if (typeof value !== "string")
        return null;
    const trimmed = value.trim();
    return trimmed || null;
}
function formatApplyTiming(value) {
    if (value === "immediate")
        return "Applied immediately to the live session.";
    if (value === "next_turn")
        return "Applies on the next turn for the active session.";
    return "Applies when the next session is created.";
}
/** Registers the agent-only session-scoped SSH configuration tool. */
export const sshTool = (pi) => {
