...
function getProviderStatuses(authStorage) {
    return PROVIDER_DEFS.map((def) => {
        ...
        if (cred?.type === "oauth")
            ...
            const models = readJsonFile(getModelsJsonPath());
...
function resolveOAuthManualInput(providerId, redirectUrl) {
...
async function handleStep2(session, authStorage, modelRegistry, registry, data) {
    const providerId = String(data.provider || "").trim();
...
async function activateProviderModel(session, modelRegistry, providerId, modelId) {
...
async function showCard3OrComplete(session, modelRegistry, def, providerId, name, registry) {
...
async function handleStep3(session, modelRegistry, data) {
    const providerId = String(data.provider || "").trim();
...
export async function handleLogin(session, modelRegistry, command) {
    ...
    if (command.provider?.startsWith("__step2 ")) {
        const json = command.provider.slice(8);
        try {
            return await handleStep2(session, authStorage, modelRegistry, registry, JSON.parse(json));
        }
        catch {
            return { status: "error", message: "Invalid data." };
        }
    ...
    if (command.provider?.startsWith("__step3 ")) {
        const json = command.provider.slice(8);
        try {
            return await handleStep3(session, modelRegistry, JSON.parse(json));
        }
        catch {
            return { status: "error", message: "Invalid data." };
        }
