...
export function getChatSshConfig(chatJid) {
    const db = getDb();
...
export function upsertChatSshConfig(config) {
    const db = getDb();
    const now = new Date().toISOString();
    const strictMode = normalizeStrictMode(config.strict_host_key_checking);
    db.prepare(`INSERT INTO chat_ssh_configs (
