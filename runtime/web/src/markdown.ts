...
function applySyntaxHighlighting(html) {
  if (!html) return html;
  if (typeof window === "undefined" || !window.hljs) return html;
  return html.replace(
    /<pre><code(?:\s+class="language-(\w+)")?>([\s\S]*?)<\/code><\/pre>/g,
    (match, lang, code) => {
      const language =
        lang && window.hljs.getLanguage(lang) ? lang : "plaintext";
...
function decodeCodeEntities(html) {
  ...
  return html
    .replace(
      ...
      (match, code) => `<pre><code>${normalize(code)}</code></pre>`,
    )
    .replace(
      /<code>([\s\S]*?)<\/code>/g,
      (match, code) => `<code>${normalize(code)}</code>`,
    );
}
...
function renderMath(html_content) {
  ...
  processed = processed.replace(
    ...
    (match, leading, tex) => {
      ...
      try {
        const rendered = katex.renderToString(decodeMath(tex), {
          displayMode: false,
          throwOnError: false,
        });
        return `${leading}${rendered}`;
...
function linkifyHashtagsInHtml(html_content) {
  ...
  for (const textNode of nodes) {
    ...
    if (
      parent &&
      (parent.closest("a") || parent.closest("code") || parent.closest("pre"))
    )
      continue;
...
export function prepareMarkdownSource(text) {
  ...
  const { text: stripped, blocks: mermaidBlocks } =
    extractMermaidBlocks(normalizedMath);
