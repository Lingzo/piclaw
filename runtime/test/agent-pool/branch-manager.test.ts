...
function createManager(
  overrides: Partial<ConstructorParameters<typeof AgentBranchManager>[0]> = {},
) {
  ...
  const manager = new AgentBranchManager({
    ...
    isActive: (chatJid) => {
      const session = pool.get(chatJid)?.runtime;
      return Boolean(
        session?.isStreaming ||
...
test("AgentBranchManager registers active chats and resolves agent handles", async () => {
  ...
  const branch = fixture.manager.ensureBranchRegistration(
    "web:topic",
    fixture.pool.get("web:topic")?.runtime,
  );
  ...
  expect(
    fixture.manager.findActiveChatByAgentName("Research Bot")?.chat_jid,
  ).toBe("web:topic");
  expect(fixture.manager.findChatByAgentName("research-bot")).toEqual({
    chat_jid: "web:topic",
    agent_name: "research-bot",
  });
  expect(fixture.manager.getAgentHandleForChat("web:topic")).toBe(
    "research-bot",
  );
...
test("AgentBranchManager prunes inactive branches and disposes cached sessions", async () => {
  ...
  const fixture = createManager();
  fixture.pool.set("web:default:branch:prune", {
    runtime: createRuntime(session),
    lastUsed: Date.now(),
  });
  fixture.sidePool.set("web:default:branch:prune", {
    runtime: createRuntime(session),
    lastUsed: Date.now(),
  });
  fixture.activeForkBaseLeafByChat.set("web:default:branch:prune", "leaf-1");

  const archived = await fixture.manager.pruneChatBranch(
    "web:default:branch:prune",
  );
  ...
  expect(fixture.activeForkBaseLeafByChat.has("web:default:branch:prune")).toBe(
    false,
  );
