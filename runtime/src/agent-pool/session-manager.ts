...
export interface AgentSessionManagerOptions {
  ...
  createDefaultTools: () => NonNullable<
    Parameters<typeof createDefaultSession>[1]["tools"]
  >;
  getSessionExtensionFactories?: (
    chatJid: string,
  ) => Promise<ExtensionFactory[]>;
  bindSession: (runtime: AgentSession, chatJid: string) => Promise<void>;
...
export class AgentSessionManager {
  ...
  async refreshRuntime(chatJid: string, runtime: AgentSession): Promise<void> {
    const entry = this.options.pool.get(chatJid);
    if (entry) {
      entry.lastUsed = Date.now();
    }
    await this.options.bindSession(runtime, chatJid);
    this.options.ensureBranchRegistration(chatJid, runtime);
  }
  ...
  async getOrCreate(chatJid: string): Promise<AgentSession> {
    ...
    const extensionFactories =
      (await this.options.getSessionExtensionFactories?.(chatJid)) ?? [];
    ...
    await this.applyDefaultModel(runtime);
    await this.refreshRuntime(chatJid, runtime);
    this.options.onInfo?.("Session ready", {
      operation: this.options.createSession
        ? "get_or_create.create_main_session"
        : "get_or_create.create_default_session",
      chatJid,
  ...
  async syncSideSessionFromMain(
    ...
    if (
      mainModel &&
      (!sideModel ||
        sideModel.provider !== mainModel.provider ||
        sideModel.id !== mainModel.id)
    ) {
      try {
  ...
  async recreate(chatJid: string): Promise<void> {
    await this.disposeEntry(
      this.options.pool,
      chatJid,
      "recreate.dispose_main_session",
    );
    await this.disposeEntry(
      this.options.sidePool,
      chatJid,
      "recreate.dispose_side_session",
      true,
    );
  }
  ...
  private async disposeEntry(
    ...
    try {
      ...
      this.options.onInfo?.(
        side ? "Disposed side session" : "Disposed session",
        {
          operation,
          chatJid,
        },
      ...
      this.options.onError?.(
        side ? "Failed to dispose side session" : "Failed to dispose session",
        {
          operation,
          chatJid,
          err,
  ...
  private async applyDefaultModel(session: AgentSession): Promise<void> {
    ...
    const sessionRegistry =
      (session as AgentSession & { modelRegistry?: ModelRegistry })
        .modelRegistry ?? this.options.modelRegistry;
    const resolved = sessionRegistry.find(provider, modelId);
    if (!resolved) return;

    const setModel = (
      session as { setModel?: (model: typeof resolved) => Promise<void> }
    ).setModel;
